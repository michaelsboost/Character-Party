function App() {
  return {
    output: {
      name: "a scared german shepherd",
      category: "dogs",
      profession: "who is a security guard"
    },
    data: {
      moods: [
        "accomplished",
    ...
    icons: {
      share: `<svg class="inline-block w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M386.3 160H336c-17.7 0-32 14.3-32 32s14.3 32 32 32H464c17.7 0 32-14.3 32-32V64c0-17.7-14.3-32-32-32s-32 14.3-32 32v51.2L414.4 97.6c-87.5-87.5-229.3-87.5-316.8 0s-87.5 229.3 0 316.8s229.3 87.5 316.8 0c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0c-62.5 62.5-163.8 62.5-226.3 0s-62.5-163.8 0-226.3s163.8-62.5 226.3 0L386.3 160z"></path></svg>`,
      heart: `<svg class="inline-block w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" data-fa-i2svg=""><path fill="currentColor" d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z"></path></svg>`
    },
    generate() {
      ...
      function searchWeb(val) {
        return `<a class="border-0 border-b border-solid pb-1" href="https://duckduckgo.com/?q=${val.toLowerCase().replace(/ /, '+')}%3F&t=h_&ia=web" target="_blank">${val}</a>`;
      }
      function searchPics(val) {
        return `<a class="border-0 border-b border-solid pb-1" href="https://duckduckgo.com/?q=${val.toLowerCase().replace(/ /, '+')}&t=h_&iax=images&ia=images" target="_blank">${val}</a>`;
      }
      function startsWithVowel(str) {
        // Regular expression to match any vowel at the beginning of the string
        return /^[aeiou]/i.test(str);
      }
      const CharactersCategory = randomNum(this.data.categories.length);
      const category = this.data.categories[CharactersCategory];
      const CharacterCategoryArray = randomNum(this.data.characters[category].length);
      const CharacterCategoryArrayItem = this.data.characters[category][CharacterCategoryArray];

      // assignment for output
      let mood = this.data.moods[randomNum(this.data.moods.length)];
      mood = startsWithVowel(mood) ? `An ${mood}` : `A ${mood}`;

      let profession = this.data.jobs[randomNum(this.data.jobs.length)];
      this.output.profession = startsWithVowel(profession) ? `Who is an ${searchWeb(profession)}` : `Who is a ${searchWeb(profession)}`;

      this.output.name = `${mood} ${searchPics(CharacterCategoryArrayItem)}`;
      this.output.category = searchWeb(category);
    }
  }
}